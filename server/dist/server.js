  ...
  "server/node_modules/vscode-languageserver-types/lib/umd/main.js"(exports2, module2) {
    (function(factory) {
      ...
      (function(Range3) {
        ...
        Range3.is = is;
      })(Range2 = exports3.Range || (exports3.Range = {}));
      var Location;
      (function(Location2) {
...
// server/node_modules/vscode-uri/lib/esm/index.mjs
var LIB;
(() => {
  "use strict";
  var t = { 975: (t2) => {
...
// server/src/viewi-parser.ts
var fs = __toESM(require("fs"));
var path = __toESM(require("path"));
var ViewiParser = class {
  constructor(workspaceRoot, searchPaths) {
    this.allComponentsCache = /* @__PURE__ */ new Map();
    this.phpFileToComponent = /* @__PURE__ */ new Map();
  ...
  async parseComponent(phpFile, htmlFile) {
    try {
      ...
      if (componentNameFromCache && this.allComponentsCache.has(componentNameFromCache) && lastPhpMtime === phpStat.mtimeMs && lastHtmlMtime === htmlStat.mtimeMs) {
...
var builtInControlTags = ["slot", "slotContent", "template"];
var viewiParser;
...
connection.onCompletion(
  async (_textDocumentPosition) => {
    ...
    function isInsideBracesWithWhitespace(document2, position2) {
      const text2 = document2.getText();
      const offset2 = document2.offsetAt(position2);
      let lastDoubleBraceOpen = -1;
      let lastDoubleBraceClose = -1;
      for (let i = offset2 - 1; i >= 1; i--) {
        if (text2.substring(i - 1, i + 1) === "{{") {
      ...
      for (let i = offset2 - 1; i >= 0; i--) {
        if (text2[i] === "{" && (i === 0 || text2[i - 1] !== "{") && (i === text2.length - 1 || text2[i + 1] !== "{")) {
...
Component property from PHP class`
    ...
    for (const method of methods) {
      const paramString = method.parameters.map((p) => `${p.type} $${p.name}${p.hasDefault ? "?" : ""}`).join(", ");
...
connection.onDefinition(
  async (params) => {
    ...
    const member = component.properties.find((p) => p.name === memberName) || component.methods.find((m) => m.name === memberName);
    if (!member) {
      const tagComponent = viewiParser.getComponent(word);
      if (tagComponent) {
        const phpDocument2 = await TextDocument.create(tagComponent.phpFile, "php", 1, await fs2.promises.readFile(tagComponent.phpFile, "utf-8"));
        const regex2 = new RegExp(`${word}\\b`);
        const match2 = regex2.exec(phpDocument2.getText());
        if (match2) {
          const startPosition = phpDocument2.positionAt(match2.index);
          const endPosition = phpDocument2.positionAt(match2.index + match2[0].length);
          return {
    ...
    const phpDocument = await TextDocument.create(component.phpFile, "php", 1, await fs2.promises.readFile(component.phpFile, "utf-8"));
